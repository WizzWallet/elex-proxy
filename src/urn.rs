...
use crate::{AppError, Callbacks, handle_request, JsonRpcRequest, R, random_callback};

pub async fn handle_urn_with_res(
    Extension(callbacks): Extension<Vec<(mpsc::UnboundedSender<JsonRpcRequest>, Callbacks)>>,
    headers:HeaderMap,
    Path((urn,res)): Path<(String,String)>,
) -> std::result::Result<R, AppError> {
    info!("URN: {},res: {}", &urn,&res);
    ...
    //         let r = handle_request(sender, calls, headers, "blockchain.atomicals.get_by_realm".into(), vec![Value::String(result.identifier)]).await;
